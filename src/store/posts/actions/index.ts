...
export const initPostsbyCategory = (params: PostParams) => {
  // eslint-disable-next-line @typescript-eslint/explicit-module-boundary-types
  return (dispatch: any) => {
    Promise.resolve(dispatch(saveCategory(params.category))).then(() =>
      dispatch(getAllPostsbyCategory(params))
    )
  }
}
...
export const getAllPostsbyCategory = createAsyncThunk<
  ...
  PostParams
>(
  POST_ACTION_TYPE.GET_POSTS_BY_CATEGORY,
  async (params, { rejectWithValue }) => {
    try {
      const res = await postServices.getAllPosts(
        params,
...
export const saveCategory = createAction<number | undefined>(SAVE_CATEGORY)
